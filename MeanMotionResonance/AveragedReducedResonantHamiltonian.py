...
class ResonanceEquations():
    ...
    def H(self,z):
        ...
        """
        return self._funcs['Hamiltonain'](z,*self.extra_args)
    ...
    def _find_conservative_equilibrium(self,guess,tolerance=1e-9,max_iter=10):
        y = guess
        lb = np.array([-np.pi,-np.pi,-np.pi, -1* y[2], -1 * y[3],-1*y[4], -np.inf])
        ub = np.array([np.pi,np.pi,np.pi, np.inf,np.inf,np.inf,np.inf])
        fun = self.H_flow
        jac = self.H_flow_jac
        f = fun(y)[:-1]
        J = jac(y)[:-1,:-1]
